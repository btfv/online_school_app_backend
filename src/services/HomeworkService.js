...
HomeworkService.getByStudent = async function (
  ...
  return await HomeworkModel.findOne({
    ...
    .then(async (result) => {
      if (!result) throw Error('Homework not found');
    ...
    .then(async (result) => {
      ...
      const tasks = result.tasks.map((task) => {
        ...
        return {
          ...
          options: task.taskType === 1 ? task.optionLabels : undefined,
        };
      });
      return {
        title: result.title,
...
HomeworkService.addTask = async function (homeworkPublicId, task, attachments) {
  ...
  await HomeworkModel.findOne(
    ...
    if (result.receivedStudents.length) {
      throw Error("You can't add task to the homework someone has received");
    }
  });
  var attachmentIds = [];
  ...
  const taskDocument = {
    ...
    attachments: attachmentIds,
    optionLabels:
      task.taskType == this.typesOfAnswers.OPTIONS_ANSWER ? task.options : [],
    optionAnswers:
      task.taskType == this.typesOfAnswers.OPTIONS_ANSWER ? task.answer : [],
    stringAnswer:
      task.taskType == this.typesOfAnswers.STRING_ANSWER ? task.answer : '',
    detailedAnswer:
      task.taskType == this.typesOfAnswers.DETAILED_ANSWER ? task.answer : '',
...
HomeworkService.removeHomework = async function (homeworkPublicId) {
  await HomeworkModel.findOne({
    ...
    .select(
      'receivedStudents receivedGroups creatorId attachments teachersWithAccess'
    )
    .then(async (homeworkDocument) => {
      ...
      teacherIds.map(async (id) => {
        await HomeworkService.removeTeacher(id, homeworkId);
      });
      /*
      ...
      await Promise.all(
        Object.entries(values).map(async ([taskPublicId, value]) => {
          await HomeworkModel.updateOne(
            ...
            {
              $set: {
                'solutions.$[i].answers.$[j].comment': value.comment,
                'solutions.$[i].answers.$[j].points': value.points,
              },
              $inc: {
                'solutions.$[i].totalPoints': parseInt(value.points),
              },
            },
            {
              arrayFilters: [
...
HomeworkService.addSolutionByStudent = async function (
  ...
  await HomeworkModel.findOne({ publicId: homeworkPublicId })
    ...
    .then(async (result) => {
      ...
      if (!tasks.length) {
        throw Error('You cant add solution to the homework without tasks');
      }
      ...
      tasks.map((task) => {
        ...
        switch (task.taskType) {
          case this.typesOfAnswers.OPTIONS_ANSWER:
            task.optionAnswers = task.optionAnswers.map((option) => {
              return option || false;
            });
            studentAnswer.answer = studentAnswer.answer.map((option) => {
              return option || false;
            });
...
HomeworkService.getSolutionByStudent = async function (
  ...
  const solutionDocument = await HomeworkModel.findOne(
    ...
    .then(async (result) => {
      if (!result) {
        throw Error('homework or solution not found');
      }
      result = result.toObject();
      if (!result) {
        throw Error("Can't find homework");
    ...
    .then(async (result) => {
      ...
      return await Promise.all(
        teachers.map(async (teacherId) => {
          ...
          return {
            ...teacherInfo,
            status: teacherId == result.creatorId ? 'Creator' : 'Revisor',
          };
        })
      );
    });
