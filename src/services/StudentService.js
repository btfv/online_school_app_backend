...
StudentService.changePassword = async function (
  ...
  const passwordsMatch = await bcrypt.compare(oldPassword, studentPasswordHash);
  if (!passwordsMatch) {
...
StudentService.getListOfStudents = async function (sliceNumber) {
  const userDocuments = await StudentModel.find({}, '-_id publicId name').limit(
    COUNT_OF_USERS_IN_QUERY
  );
  return userDocuments;
};
